...
import {getInfoClubPage, getInfoYear} from "../../action/clubActions";
import { ClubPageType,StateType} from '../../common/types'


const Club:FC<ClubPageType> = () => {
    const dispatch = useDispatch()
    const numberYears = useSelector((state:StateType) => state.clubPage.infoForClub)
    const listYears = useSelector((state:StateType) => state.clubPage.listYears)
    useEffect(() => {
        dispatch(getInfoClubPage())
    }, [dispatch])
    const currentActiveClass = (id:number) => {
